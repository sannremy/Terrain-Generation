...
<body>
<div id="title">Terrain Generation using the diamond-square algorithm, Three.js (WebGL)</a> - <a href="http://srchea.com/terrain-generation-the-diamond-square-algorithm-and-three-js">Read the related blog post</a><br /><span id="debug"></span></div>
<div id="panel">
	<ul>
		<?php $optionsWidth = ''; for($i = 2; $i <= 10000; $i++) $optionsWidth .= '<option value="'.$i.'"'.($i === 1000 ? ' selected="selected"' : '').'>'.$i.' px</option>'; ?>
		<?php $optionsHeight = ''; for($i = 2; $i <= 10000; $i++) $optionsHeight .= '<option value="'.$i.'"'.($i === 500 ? ' selected="selected"' : '').'>'.$i.' px</option>'; ?>
		<li><label>Width<br /><select id="width"><?php echo $optionsWidth; ?></select></label></li>
		<li><label>Height<br /><select id="height"><?php echo $optionsHeight; ?></select></label></li>
		...
		<li><label>Ground <input type="checkbox" id="border" checked="checked" /></label></li>
